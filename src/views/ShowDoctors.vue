...
<div>
  <div class="position-relative">
        <input
          ...
          placeholder="Search Doctor" v-model="searchBar" @focus="showSearchlist=true" @blur="showSearchlist=false"
        />
        <button class="btn btn-light p-4 rounded-end-pill border">🔍</button>

        <div v-if="showSearchlist" class="position-absolute bg-white border rounded" style="z-index: 9999; left:200px; top: 122px;">
  ...
  <img src="@/assets/filter.svg" alt="filter" style="width: 20px;" class="me-1 ms-3 mb-1">
  <select @change="filter($event.target.value);" name="Filter By Specialty" id="specs">
    ...
    <div v-if="filterBoolean">
       <div class="row justify-content-center align-items-center p-0 m-0 mt-5">
          <div class="card shadow-lg col-7 col-sm-6 col-md-5 col-lg-2 me-3 ms-4 g-3"
...
<div class="mb-5"></div>
  ...
  <button v-show="currpage!=1" @click="currpage--" class="btn btn-lg btn-primary mb-5"> « </button>
  <button class="btn btn-lg btn-primary mb-5">{{ currpage }}</button>
  <button v-show="currpage < pages.length" @click="currpage++" class="btn btn-lg btn-primary mb-5"> » </button>
</div>
</div>
</template>
...
export default {
  ...
  methods: {
    async getDoctors() {
      await fetch(`https://mocki.io/v1/5bdf1b60-1396-4fb7-9248-bffdbc33066f`)
        .then((res) => res.json())
        .then((data) => {
          this.doctors = data;
...
//time validation
  ...
  computed: {
   filteredSearch(){
      return this.doctors.filter((item)=> item.name.toLowerCase().includes(this.searchBar.toLowerCase()));
   },
   ...
   appearedDoctors(){
     let skip = ((this.currpage-1))* this.loadPerPage;
      let appearedDoctors= this.doctors.slice( skip, this.currpage*this.loadPerPage);
