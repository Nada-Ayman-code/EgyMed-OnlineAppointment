  ...
  <div id="app">
    ...
    <main>
      ...
      <div
        ...
        class="border border-warning position-fixed p-4"
        style="
          right: 70px;
          bottom: 50px;
          background-color: rgba(255, 255, 143, 0.4);
          width: 195px;
          z-index: 9999;
        "
      >
        <div v-for="time in cart" :key="time.doctor.id">
          Appointment with {{ time.doctor.name }}<br />
          {{ time.timer }}<br />
          <hr />
        </div>
      </div>
     ...
     this.cart= storedCart;
      let rep = setInterval(() => {
        ...
        this.cart.forEach((item) => {
          ...
          item.timer = `${days}d ${hrs}h: ${mins}m: ${secs}s`;
        });
      }, 1000);
    },
  },
